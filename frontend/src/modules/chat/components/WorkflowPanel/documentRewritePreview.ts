...
export function documentRewritePreview(value: unknown, revision: number, draft?: number): RewriteSelectionPreview {
  ...
  if (!Array.isArray(data.results) || data.results.length !== 1) throw new Error('expected one rewrite target');
  const item = record(data.results[0]), target = record(item.target), preview = record(item.preview);
  ...
  return {
    status:'ready', action:'rewrite_selection', base_revision:revision, base_draft_version:draft, representation,
