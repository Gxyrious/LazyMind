...
def test_wechat_prompt_title_not_found(monkeypatch):
    ...
    with pytest.raises(ToolExecutionError, match='请输入草稿文章的准确完整标题'):
...
def _failing_cover_generator(*args, **kwargs):
    raise RuntimeError('model failed')


@pytest.mark.parametrize(
    ('model_available', 'generator'),
    [
        (lambda role: False, None),
        (lambda role: True, _failing_cover_generator),
    ],
    ids=('unavailable', 'generation-failed'),
)
def test_new_wechat_draft_falls_back_to_white_cover(
    tmp_path: Path, model_available, generator,
):
    target = _prepare_wechat_cover(
